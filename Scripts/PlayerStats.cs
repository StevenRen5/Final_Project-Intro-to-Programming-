...
{ // variables
    ...
    public int _playerDonut = 0; // player coin tracker
    public int _playerSushi = 0;
    ...
    private void OnTriggerEnter2D(Collider2D collision) // collisions with different objects
    {
        ...
        switch(colTag)  // similar to if statement but does everything within { } by doing case
        {
            ...
            case Structs.Tags.donutTag:
                {

                    _playerDonut++; // increases player coin count by 1
                    _UIController.DonutTextUpdate(_playerDonut); // updates coin count
                    _audioSourceController.PlaySFX(Structs.SoundEffects.donut); // audiosource controller plays sound effects when player touched the coin (COIN AUDIO)
                    Destroy(collision.gameObject); // Destroys coin after player touches it
                    return;

                }

            case Structs.Tags.sushiTag:
                {

                    _playerSushi++; // increases player coin count by 1
                   // _UIController.SushiTextUpdate(_playerSushi); // updates coin count
                    _audioSourceController.PlaySFX(Structs.SoundEffects.sushi); // audiosource controller plays sound effects when player touched the coin (COIN AUDIO)
                    Destroy(collision.gameObject); // Destroys coin after player touches it
                    return;
