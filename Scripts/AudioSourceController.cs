...
public class AudioSourceController : MonoBehaviour
{
    ...
    public GameObject donutSFX; // object
    public GameObject sushiSFX;
    ...
    private void Start()
    {
        if (!PlayerPrefs.HasKey(Structs.Mixers.sfxVolume))
        {
            PlayerPrefs.SetFloat(Structs.Mixers.sfxVolume, 1);
        }

        if (!PlayerPrefs.HasKey(Structs.Mixers.musicVolume))
        {
            PlayerPrefs.SetFloat(Structs.Mixers.musicVolume, 1);
        }
    ...
    public GameObject GetSFX(string audioName)
    {
        switch (audioName)
        {
            case Structs.SoundEffects.donut:
                {
                    return donutSFX;
                }
            case Structs.SoundEffects.sushi:
                {
                    return donutSFX;
                }
